...
Runs locally on the single phone. Same REST API as the multi-phone version,
but self-registers the local node on startup.
...
def sweeper():
    while True:
        ...
        with _db_lock, get_db() as db:
            ...
            for row in gone:
                db.execute("UPDATE nodes SET online=0 WHERE node_id=?", (row["node_id"],))
                db.execute(
...
###############################################################################
# Self-registration (solo node registers itself)
###############################################################################
...
def self_register():
    ...
    with _db_lock, get_db() as db:
        ...
        if existing:
            db.execute(
                "UPDATE nodes SET role=?,ip=?,port=?,last_seen=?,online=1 WHERE node_id=?",
                (role, "127.0.0.1", 8080, now, node_id)
            )
        else:
            db.execute(
                "INSERT INTO nodes(node_id,role,ip,port,registered_at,last_seen,online) VALUES(?,?,?,?,?,?,1)",
...
###############################################################################
# Self-heartbeat thread (solo node keeps itself alive in the DB)
###############################################################################
...
def read_cpu_percent():
    """Single-sample CPU usage via /proc/stat."""
    try:
        ...
        return round(100.0 * (delta_total - delta_idle) / delta_total, 1) if delta_total else 0.0
    except Exception:
        return None
...
def register():
    ...
    now = time.time()
    with _db_lock, get_db() as db:
        exists = db.execute("SELECT node_id FROM nodes WHERE node_id=?", (node_id,)).fetchone()
        if exists:
            db.execute(
                "UPDATE nodes SET role=?,ip=?,port=?,last_seen=?,online=1,meta=? WHERE node_id=?",
                (role, ip, port, now, meta, node_id)
            )
            ev = "re-registered"
        else:
            db.execute(
                "INSERT INTO nodes(node_id,role,ip,port,registered_at,last_seen,online,meta) VALUES(?,?,?,?,?,?,1,?)",
...
def heartbeat():
    ...
    with _db_lock, get_db() as db:
        ...
        db.execute("UPDATE nodes SET last_seen=?,online=1 WHERE node_id=?", (now, node_id))
        db.commit()
...
def status():
    with _db_lock, get_db() as db:
        ...
        online    = db.execute("SELECT COUNT(*) c FROM nodes WHERE online=1").fetchone()["c"]
        offline   = db.execute("SELECT COUNT(*) c FROM nodes WHERE online=0").fetchone()["c"]
        events    = db.execute("SELECT * FROM events ORDER BY ts DESC LIMIT 30").fetchall()
    return jsonify({
...
def events():
    ...
    with _db_lock, get_db() as db:
        rows = db.execute("SELECT * FROM events ORDER BY ts DESC LIMIT ?", (limit,)).fetchall()
    return jsonify([dict(r) for r in rows])
